...
namespace Peach.Host.Controllers
{
   ...
   // [ServiceFilter(typeof(VodValidationFilter))] // 添加自定义的验证过滤器
    public class VodController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonObject> HomeAsync(string pwd, string rule)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonObject> HomeAsync(string pwd, string rule, string t, string pg, string ac, string f)
        {
            if (!pwd.ToLower().Equals(apiPwd.ToLower()))
                throw new BusinessException("接口密码错误！");
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<DetailsDto> HomeAsync(string pwd, string rule, string ids, string ac)
        {
            if (!pwd.ToLower().Equals(apiPwd.ToLower()))
                throw new BusinessException("接口密码错误！");
        ...
        /// <returns></returns>
        [HttpGet("{wd}")]
        public async Task<ClassifyDto> SearchAsync(string pwd, string rule, string wd)
        {
            if (!pwd.ToLower().Equals(apiPwd.ToLower()))
                throw new BusinessException("接口密码错误！");
        ...
        public async Task<string> SniffingAsync(string pwd, string rule, string play_url)
        {
            if (!pwd.ToLower().Equals(apiPwd.ToLower()))
                throw new BusinessException("接口密码错误！");
            return await vodInfoService.SniffingAsync(new VodInput());
        }
